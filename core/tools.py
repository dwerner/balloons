...
def _simplify_schema_for_gemini(schema: dict | None) -> dict | None:
    """Simplify a JSON Schema for Gemini compatibility.

    Gemini's FunctionDeclaration doesn't support all JSON Schema features.
    This removes/simplifies unsupported constructs like 'oneOf'.

    Args:
        schema: JSON Schema dict (may be modified in place)

    Returns:
        Simplified schema, or None if input was None
    """
    if schema is None:
        return None
    ...
    def simplify_property(prop: dict) -> dict:
        ...
        # Handle 'oneOf' - convert to a simple string type with description
        if "oneOf" in prop:
            # Combine descriptions from all options
            descriptions = []
            if "description" in prop:
                descriptions.append(prop["description"])
            for option in prop["oneOf"]:
                if "description" in option:
                    descriptions.append(option["description"])
            # Replace with a simple string type
            return {
                "type": "string",
                "description": " | ".join(descriptions) if descriptions else "Complex type (see tool documentation)",
            }
        ...
        if prop.get("type") == "array" and "items" in prop:
            if isinstance(prop["items"], dict):
                prop["items"] = simplify_property(prop["items"])
...
def get_tools_for_gemini(
    ...
    Converts the OpenAI-format tool definitions to Gemini's format.
    The Gemini SDK uses types.Tool with function_declarations.

    Note: Some JSON Schema features (oneOf, anyOf) are not supported by
    Gemini and are simplified during conversion.

    Args:
        Same as get_tools_for_request()

    Returns:
        List containing a single types.Tool with all function declarations,
        or None if tools disabled
    """
    if disable_tools:
        return None

    # Get OpenAI-format tools
    openai_tools = get_tools_for_request(
        allowed_tools=allowed_tools,
        disable_tools=False,
        include_balloon_tools=include_balloon_tools,
        include_supervisor_tools=include_supervisor_tools,
        include_review_tools=include_review_tools,
        include_goal_tools=include_goal_tools,
        include_midi_tools=include_midi_tools,
    ...
    # Import Gemini types (lazy to avoid import if not using Gemini)
    try:
    ...
    # Convert each OpenAI tool definition to Gemini FunctionDeclaration
    function_declarations = []
    for tool in openai_tools:
        func = tool["function"]
        # Simplify schema for Gemini compatibility
        parameters = _simplify_schema_for_gemini(func.get("parameters"))
    ...
    # Return as a list containing one Tool with all declarations
