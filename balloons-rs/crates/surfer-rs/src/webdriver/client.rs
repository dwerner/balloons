...
use crate::webdriver::actions::Actions;
use crate::webdriver::elements::{Element, Form};
use crate::webdriver::error;
use crate::webdriver::session::{Cmd, Session};
use crate::webdriver::wait::Wait;
use crate::webdriver::wd::{
...
///
/// Use [`ClientBuilder`](crate::webdriver::ClientBuilder) to create a new session.
///
...
#[derive(Clone)]
pub struct Client {
    pub(crate) tx: Sender<crate::webdriver::session::Task>,
    pub(crate) new_session_response: Option<NewSessionResponse>,
    /// Handle to the background session thread - must be kept alive
    pub(crate) _session_handle: Arc<std::thread::JoinHandle<()>>,
}
...
impl std::fmt::Debug for Client {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Client")
            .field("tx", &self.tx)
            .field("new_session_response", &self.new_session_response)
            .finish()
    }
}
...
impl Client {
    ...
    ///
    /// Prefer using [`ClientBuilder`](crate::webdriver::ClientBuilder) over calling this method directly.
    #[deprecated(since = "0.17.1", note = "Prefer ClientBuilder::smol")]
    pub async fn new(webdriver: &str) -> Result<Self, error::NewSessionError> {
        crate::webdriver::ClientBuilder::smol()
            .connect(webdriver)
            .await
    }
    ...
    pub(crate) async fn new_with_connector(
        ...
        Session::with_capabilities(webdriver, &Default::default()).await
    }
    ...
    ///
    pub async fn with_existing_session(
        webdriver: &str,
        session_id: &str,
    ) -> Result<Self, error::NewSessionError> {
        let (client, wdb) = Session::create_client_and_parse_url(webdriver).await?;
    ...
    /// `Client` is dropped. Specifically, the shutdown request will be issued by the background session thread when the last
    /// client handle is dropped. Any errors during that implicit shutdown are not surfaced to the
    /// caller.
    ///
...
///
/// ```ignore
/// use surfer_rs::webdriver::{ClientBuilder, Locator};
///
/// # async fn demo() -> Result<(), surfer_rs::webdriver::error::CmdError> {
...
/// builder.method(http::Method::GET).url(&src);
/// builder.skip_cookie_navigation();
/// let _raw = builder.send().await?;
///
/// client.close().await
/// # }
/// ```
...
impl<'a> RawRequestBuilder<'a> {
    ...
    /// Add or replace an HTTP header on the raw request.
    pub fn header(&mut self, key: http::header::HeaderName, value: http::HeaderValue) -> &mut Self {
    ...
    pub async fn send(self) -> Result<ResponseData, error::CmdError> {
...
impl Client {
    ...
    pub async fn raw_client_for(
        ...
        method: Method,
        url: &str,
    ) -> Result<ResponseData, error::CmdError> {
    ...
    pub async fn with_raw_client_for(
        &self,
        method: Method,
        url: &str,
    ) -> Result<ResponseData, error::CmdError> {
...
impl Client {
    ...
    /// # use surfer_rs::webdriver::{ClientBuilder, Locator};
    /// # async fn demo() -> Result<(), surfer_rs::webdriver::error::CmdError> {
    /// # let client = ClientBuilder::smol().connect("http://localhost:4444").await.expect("failed to connect to WebDriver");
    /// // -- snip wrapper code --
