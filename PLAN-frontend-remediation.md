...
- **WS7 — session deep-links (partial)**: `utils/urlRouting.ts` (pure parse/format/prefix-match) + `hooks/useUrlRouting.ts` (initialRoute seed, replaceState mirror, reactive currentRoute) + 21 tests. Connect handler prefers a prefix-matched `#/sessions/:id[/:tab]` deep link; a selection→URL effect mirrors session-tab changes. **Back/forward intentionally deferred** — a reactive currentRoute→selection effect can bounce a click back to the previous session (stale-route re-run); the bounce-proof shape is a hook-pushed nav callback + live QA. Suite 74 → 95.
