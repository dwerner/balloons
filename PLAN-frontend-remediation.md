...
- **WS3 (cont. 2)**: post-chrome-extraction dead-code sweep — dead `turns` prop on `SidebarContent`, module-level `globalClient` (written, never read), the debug-toggle cluster, `uploadImage`, two dead voice handlers, and ~15 now-unused imports. **App.tsx 2885 → 2816 lines.**
- **WS7 — session + global-tab deep-links (complete for those families)**: `utils/urlRouting.ts` (pure parse/format/prefix-match) + `hooks/useUrlRouting.ts` (initialRoute seed, replaceState mirror, popstate/hashchange nav callback) + 30 tests. Connect handler prefers a prefix-matched `#/sessions/:id[/:tab]` deep link; a selection→URL effect mirrors session-tab changes; browser back/forward drives selection via a nav callback that fires only on genuine popstate/hashchange (replaceState never fires it → no feedback loop). **Global tabs** (`#/code`, `#/logs`, `#/llm`, `#/settings`, `#/surveys`) added: `GlobalTab` type + `isGlobalTab` + `formatGlobalRoute`, `replaceGlobalTab`, and App wiring (global initial route sets the tab on connect; nav callback maps a global route to the tab; mirror effect writes `#/<tab>`). Loop-safe: the mirror effect and nav handler are mutually exclusive by tab kind. Suite 74 → 97 → 106. **Live-verified** (headless Chrome): `#/settings` deep-link-on-load lands on Settings; clicking Code mirrors to `#/code`; navigating back to `#/settings` reactivates Settings; no oscillation. Goal / turn route families remain future work (parse to `default`).
...
- **WS4 — single source of truth**: introduce a store to retire the child→parent→sibling turn round-trip and the parallel `TurnInfo`/`SessionDataTurn` state (Bugs #13, #18, #22, #9). Highest bug leverage; the `useSessionData` characterization tests are now in place, but the refactor still needs live streaming QA. Note: App's `turns` state is still dual-written by the two live `onToolUseStarted`/`onToolUse` handlers plus `handleTurnsChange` — that dual-ownership is the crux of WS4.
  - **Finding (2026-09-04, static trace): the dual-write is load-bearing, not redundant.** The backend emits tool_use turns with `role="assistant"` + `content_block_type="tool_use"` (session_manager_service.py `tool_use_turn_started`). In `useSessionData`, assistant turns stay **pending** until `turnFinished` materializes them, and the `sessionDataToolUse*` handlers only patch already-materialized turns in `turnsById` — so a streaming tool_use turn is **absent from `useSessionData.turns`** until it finishes. The two `client.tasks.*` handlers in App patch `turns` during exactly that window so the sidebar/ContextTabView render the in-flight tool call. Deleting them (the naive "dedup" move) would blank the sidebar/context for tool calls mid-stream. A correct WS4 must first make `useSessionData` surface pending tool_use turns (or have the sidebar read the pending+tool state directly), then retire the handlers — verified against a live stream.
...
- **WS7 — caching + routing**: IndexedDB cache (Bug #5), lazy tree turn-loading (Bug #4). Session + global-tab URL routing is done (see above); goal / turn route families remain.
  - **Finding (2026-09-04, Bug #4): the lazy-load hook exists but is unwired.** `App.handleLoadTurns` (loads a session's turns via `loadSessionWithLayers(client, id, clientId, ['header','body','history'])`) is passed to `SidebarContent` as `onLoadTurns`, but `SidebarContent` destructures it and never forwards it, and `SessionTreeView` has no `onLoadTurns`/expand-load prop at all — so expanding/selecting a non-loaded session never triggers a load, which is exactly the "shows empty" symptom. The fix is to forward `onLoadTurns` into `SessionTreeView` and call it on expand/select of an unloaded node, then render the returned turns — but the tree's expand semantics (does it render turns inline vs. only child forks?) must be confirmed against the live app before touching this 1136-line component. Left as the concrete Bug #4 fix point; `handleLoadTurns` is intentionally kept (deleting it would remove the building block).
