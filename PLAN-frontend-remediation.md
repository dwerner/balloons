...
- **WS7 — session deep-links (complete for session routes)**: `utils/urlRouting.ts` (pure parse/format/prefix-match) + `hooks/useUrlRouting.ts` (initialRoute seed, replaceState mirror, popstate/hashchange nav callback) + 23 tests. Connect handler prefers a prefix-matched `#/sessions/:id[/:tab]` deep link; a selection→URL effect mirrors session-tab changes; browser back/forward drives selection via a nav callback that fires only on genuine popstate/hashchange (replaceState never fires it → no feedback loop). Suite 74 → 97. Global-tab / goal / turn route families remain future work (parse to `default`).
